...
float4 CloudsPS(SkyVertOut IN) : COLOR0 {
    ...
    // Calculate factors related to sun position
    ...
    float4 tur = tex2Dlod(sampBaseTex, float4(IN.texcoords * dis1 + float2(time * 0.005, time * 0.005), 0, 0));
    float4 tur2 = tex2Dlod(sampBaseTex, float4(IN.texcoords * dis2 + float2(time * 0.005, time * 0.005), 0, 0));
    ...
    c = incol * lerp(ca, clouds, sunCol.r * 0.5);
